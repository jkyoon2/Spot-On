...
"""
output_path = "run_on_video/example/output.mp4"
        probe = ffmpeg.probe(video_path)
        video_length = float(probe['format']['duration'])
        target_duration = 145.0
        speed_factor = 1 / (video_length / target_duration)
        
        stream = ffmpeg.input(video_path)
        stream = ffmpeg.filter_(stream, 'setpts', f'{speed_factor}*PTS')
        stream = ffmpeg.output(stream, output_path, an=None, t=145.0)
        ffmpeg.run(stream)
"""
def modify_length(video_path, output_path, new_length=145.0):
    probe = ffmpeg.probe(video_path)
    video_length = float(probe['format']['duration'])
    speed_factor = 1 / (video_length / new_length)
